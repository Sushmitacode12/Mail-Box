...
function Inbox() {
    ...
    const submitHandler = ()=>{
        ...
        fetch(`https://mail-box-client-63f1e-default-rtdb.firebaseio.com/Email/${localStorage.getItem('email')}/Recieve.json`).then((res)=>{
            if(res.ok){
                return res.json()
            }else{
       ...
       const handleclick = (id)=>{
        fetch(`https://mail-box-client-63f1e-default-rtdb.firebaseio.com/Email/${localStorage.getItem('email')}/Recieve/${id}.json`,{
            method:"PATCH",
            body:JSON.stringify({
                show:false
            }),
